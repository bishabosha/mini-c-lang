...
@main def Parser(args: String*): Unit = {
  ...
  MyCCLib // force it to load

  timeIt("GRAAL_CONTEXT_STARTUP")

  MyCCLib.setDebug(debug)
  MyCCLib.initSymbTable()

  for t <- MyCCLib.getAst do try {
    timeIt("LEXING_TIME")
    val identPool = MyCCLib.identPool
